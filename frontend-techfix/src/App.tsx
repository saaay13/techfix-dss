...
function Sidebar() {
  ...
  return (
    <aside className="w-64 bg-card border-r border-border min-h-screen p-4">
      <div className="mb-8">
        <h1 className="text-xl font-bold text-primary-600">TechFix DSS</h1>
      </div>
      ...
      <div className="absolute bottom-4 left-4 right-4 border-t border-border pt-4">
        ...
        <button
          onClick={logout}
          className="mt-2 w-full px-3 py-1.5 bg-error/10 text-error rounded-lg text-xs font-medium hover:bg-error/20 transition-colors"
        >
          Cerrar Sesión
        </button>
      </div>
    ...
    <BrowserRouter>
      <AuthProvider>
        <Routes>
          ...
          <Route path="/usuarios/nuevo" element={<PrivateRoute><ProtectedRoute roles={['Administrador']}><Layout><UserFormPage /></Layout></ProtectedRoute></PrivateRoute>} />
          <Route path="/usuarios/:id/editar" element={<PrivateRoute><ProtectedRoute roles={['Administrador']}><Layout><UserFormPage /></Layout></ProtectedRoute></PrivateRoute>} />
